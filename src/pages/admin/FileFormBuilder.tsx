...
function BuilderInner() {
  ...
  const { templateIdParam } = useParams();
  const { user } = useAuth();
  ...
  React.useEffect(() => {
    ...
    loadTemplate();
  }, [templateIdParam, setFields, setFormName, setFormDescription]);
      ...
      <div className="h-screen bg-bg-primary flex flex-col font-sans overflow-hidden">
        ...
        <header className="h-14 bg-bg-secondary border-b border-bg-border flex items-center justify-between px-6 shrink-0">
          <div className="flex items-center gap-4">
            <button onClick={() => navigate('/admin/file-forms')} className="text-text-muted hover:text-white transition-colors">
...
export default function FileFormBuilder() {
  return (
    <FormBuilderProvider>
      <BuilderInner />
    </FormBuilderProvider>
