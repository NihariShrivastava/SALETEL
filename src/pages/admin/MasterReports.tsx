...
export default function MasterReports() {
  ...
  const renderSummaryCards = () => {
    if (activeTab === 'teamlead') {
      ...
      const totalEntries = dataTeamLeads.reduce((acc, curr) => acc + curr.totalEntries, 0);
      const totalAssigned = dataTeamLeads.reduce((acc, curr) => acc + curr.assigned, 0);
      const totalClosed = dataTeamLeads.reduce((acc, curr) => acc + curr.closed, 0);

      return (
        <>
          <Card className="p-4">
            <div className="text-xs text-text-secondary uppercase tracking-wider font-semibold mb-1">Total Team Leads</div>
            <div className="text-2xl font-bold text-white tracking-tight">{totalTLs.toLocaleString()}</div>
            <div className="text-xs text-text-muted mt-1 flex items-center">Active team managers</div>
          </Card>
          <Card className="p-4">
            <div className="text-xs text-text-secondary uppercase tracking-wider font-semibold mb-1">Total Entries Managed</div>
            <div className="text-2xl font-bold text-white tracking-tight">{totalEntries.toLocaleString()}</div>
            <div className="text-xs text-text-muted mt-1 flex items-center">Entries under these leads</div>
          </Card>
          <Card className="p-4">
            <div className="text-xs text-text-secondary uppercase tracking-wider font-semibold mb-1">Assigned To TC</div>
            <div className="text-2xl font-bold text-white tracking-tight">{totalAssigned.toLocaleString()}</div>
            <div className="text-xs text-text-muted mt-1 flex items-center">Leads pushed to callers</div>
          </Card>
          <Card className="p-4">
            <div className="text-xs text-text-secondary uppercase tracking-wider font-semibold mb-1">Total Closed</div>
            <div className="text-2xl font-bold text-accent-green tracking-tight">{totalClosed.toLocaleString()}</div>
            <div className="text-xs text-text-muted mt-1 flex items-center">Successfully closed leads</div>
          </Card>
        </>
      );
    }
    ...
    if (activeTab === 'telecaller') {
      ...
      return (
        <>
          <Card className="p-4">
            <div className="text-xs text-text-secondary uppercase tracking-wider font-semibold mb-1">Total Telecallers</div>
            <div className="text-2xl font-bold text-white tracking-tight">{totalTCs.toLocaleString()}</div>
            <div className="text-xs text-text-muted mt-1 flex items-center">Active telecallers</div>
          </Card>
          <Card className="p-4">
    ...
    if (activeTab === 'surveyor') {
      ...
      const sorted = [...dataSurveyors].sort((a,b) => b.submissions - a.submissions);
      const topSurveyor = sorted[0]?.name || 'N/A';
      const avgSubs = totalSurveyors > 0 ? (totalSubs / totalSurveyors).toFixed(1) : '0';

      return (
        <>
          <Card className="p-4">
            <div className="text-xs text-text-secondary uppercase tracking-wider font-semibold mb-1">Total Surveyors</div>
            <div className="text-2xl font-bold text-white tracking-tight">{totalSurveyors.toLocaleString()}</div>
            <div className="text-xs text-text-muted mt-1 flex items-center">Active data collectors</div>
          </Card>
          <Card className="p-4">
            <div className="text-xs text-text-secondary uppercase tracking-wider font-semibold mb-1">Total Collections</div>
            <div className="text-2xl font-bold text-white tracking-tight">{totalSubs.toLocaleString()}</div>
            <div className="text-xs text-text-muted mt-1 flex items-center">Forms submitted by these surveyors</div>
          </Card>
          <Card className="p-4">
            <div className="text-xs text-text-secondary uppercase tracking-wider font-semibold mb-1">Average per Surveyor</div>
            <div className="text-2xl font-bold text-accent-blue tracking-tight">{avgSubs}</div>
            <div className="text-xs text-text-muted mt-1 flex items-center">Submissions per surveyor</div>
          </Card>
          <Card className="p-4">
    ...
    // Default for master, domain, role
    return (
      <>
        <Card className="p-4">
