...
export default function AdminAssignedCustomDashboard() {
  ...
  const [allTelecallers, setAllTelecallers] = useState<any[]>([]);
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      // 1. Fetch all templates for template switcher
      const { data: tmplList } = await supabase
        .from('form_templates')
        .select('id, name')
        .or('is_deleted.is.null,is_deleted.eq.false')
        .order('name');
      setAllTemplates(tmplList || []);
      ...
      if (templateData) {
        ...
        // Fallback to file_form_templates
      ...
      // 3. Fetch ALL assigned submissions for this template (Admin has global view)
      ...
      // 4. Fetch all telecallers in the system for reassignment
      const { data: surveyorsData } = await supabase
        .from('surveyors')
        .select('id, full_name, username, user_role:user_roles(name)')
        .order('full_name');

      if (surveyorsData) {
        const tcs = surveyorsData.filter(u => {
          const roleName = Array.isArray(u.user_role) ? u.user_role[0]?.name : (u.user_role as any)?.name;
          return roleName?.toLowerCase().includes('telecaller');
        });
        setAllTelecallers(tcs);
      }
    } catch (err: any) {
      console.error('Failed to load admin assigned leads data:', err);
  ...
  const handleReassign = async (targetTcId?: string) => {
    ...
    try {
      ...
      const tcObj = allTelecallers.find(t => t.id === targetId);
  ...
  const handleExportExcel = () => {
    ...
    XLSX.writeFile(workbook, `${template.name}_Admin_Assigned_Leads_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
    ...
    <div className="space-y-6 max-w-[1600px] mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-bg-secondary p-6 rounded-2xl border border-bg-border relative overflow-hidden">
        ...
        <div className="flex items-center gap-4 relative z-10">
          <Button 
            variant="outline" 
            size="sm" 
            onClick={() => navigate('/admin/assigned-leads')} 
          ...
          <div>
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {allTemplates.length > 1 && (
                <div className="flex items-center gap-2">
                  <span className="text-xs text-text-muted">Switch Form:</span>
                  <select
                    value={templateId}
                    onChange={e => navigate(`/admin/assigned-leads/${e.target.value}`)}
                    className="bg-bg-primary border border-bg-border rounded-lg px-2.5 py-1 text-xs text-white focus:border-accent-blue focus:outline-none"
                  >
                    {allTemplates.map(t => (
            ...
            <p className="text-text-secondary text-sm mt-1">
              Admin Workspace: Filter assigned leads by question answers, inspect telecaller assignments, and reassign leads.
            </p>
          </div>
        </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-24 gap-3 text-text-muted">
          ...
          <span className="text-sm">Loading admin assigned leads analysis...</span>
        </div>
      ) : template ? (
          ...
          <Card className="w-full max-w-md bg-bg-secondary p-0 overflow-hidden shadow-2xl border-bg-border">
            <div className="p-4 border-b border-bg-border flex justify-between items-center bg-bg-primary">
              <h3 className="font-bold text-white text-base flex items-center gap-2">
                ...
                {singleReassignSub ? 'Reassign Lead (Admin)' : `Reassign ${selectedIds.size} Leads (Admin)`}
              </h3>
              <button 
                onClick={() => {
                  setIsReassignModalOpen(false);
